...
def plot_track_embeddings(track_embeddings, n_components=2, title="Track Embeddings"):
  ...
  markers = ['red', 'green', 'blue', 'purple', 'black', 'black', 'magenta', 'black', 'red', 'pink', 'black']  # Choose 10 markers
#   n = 0
  for i, (track_id, embeddings) in enumerate(track_embeddings.items()):
    reduced_embeddings = pca.transform(embeddings)
    plt.scatter(reduced_embeddings[:, 0], reduced_embeddings[:, 1], label=track_id, c=markers[int(track_id)-1])

    # plt.scatter(reduced_embeddings[i][0], reduced_embeddings[i][1], label=track_id, c=markers[int(track_id)])
            ...
            for result in results.boxes.cpu().numpy():
                if result.id:
                    ...
                    if track_id not in all_features.keys():
                        all_features[track_id] = [embedding]
                    else:
                        l = all_features[track_id]
                        l.append(embedding)
                        all_features[track_id] =l
...
plot_track_embeddings(all_features,n_components=2)
